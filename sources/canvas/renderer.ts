...
/** Increments every time `runRenderCharacter` finishes drawing to the offscreen canvas. */
export let offscreenVersion = 0;
...
export async function renderCharacter(
  ...
  // Clone selections to prevent stale data from mutations during async renders
  const frozenSelections: Selections = JSON.parse(JSON.stringify(selections));
  const p = renderCharacterSerial.then(() =>
    runRenderCharacter(catalog, frozenSelections, bodyType, targetCanvas),
  );
...
async function runRenderCharacter(
  ...
  try {
    ...
    // Resize canvas to fit all content (only reset if dimensions changed to avoid context reset)
    if (renderCanvas.width !== totalWidth || renderCanvas.height !== totalHeight) {
      renderCanvas.width = totalWidth;
      renderCanvas.height = totalHeight;
      // Re-apply context settings that canvas reset clears
    ...
    for (const { item, img, success } of loadedItems) {
      if (success && img) {
        try {
          ...
          debugWarn(`Failed to draw item ${item.itemId} layer ${item.layerNum}: ${e}`);
        }
      }
    }
        ...
        for (const { item: areaItem, img, success } of loadedCustomImages) {
          if (success && img) {
            try {
              ...
              debugWarn(`Failed to draw custom animation item ${areaItem.itemId}: ${e}`);
            }
          }
        }
      }
      ...
      for (const { item: sprite, img, success } of loadedImages) {
        if (success && img) {
          try {
            ...
            debugWarn(`Failed to draw item ${itemId} sprite: ${e}`);
          }
        }
      }
    },
      ...
      for (const { item: sprite, img, success } of loadedImages) {
        if (success && img) {
          try {
            ...
            debugWarn(`Failed to draw single animation item ${itemId}: ${e}`);
          }
        }
      }
    },
