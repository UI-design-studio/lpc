...
function paintPreviewFrameForCycleIndex(cycleIndex: number): void {
  ...
  // Background color
  if (state.previewBgColor) {
    previewCtx.fillStyle = state.previewBgColor;
    previewCtx.fillRect(0, 0, previewCanvas.width, previewCanvas.height);
  } else if (state.showTransparencyGrid) {
  ...
  // Outline: draw 1px black border around non-transparent pixels
  if (state.showOutline && previewCanvas.width > 0) {
    const w = previewCanvas.width;
    const h = previewCanvas.height;
    const imgData = previewCtx.getImageData(0, 0, w, h);
    const d = imgData.data;
    const mask = new Uint8Array(w * h); // 0 = not outline, 1 = outline
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        const idx = (y * w + x) * 4;
        if (d[idx + 3] > 0) {
          // Opaque pixel: check if any neighbor is transparent
    ...
    // Apply outline mask in a second pass (no read/write conflict)
...
export function startPreviewAnimation(): void {
  ...
  function nextFrame(): void {
    const fpsInterval = 1000 / (8 * animationSpeed);
    const now = Date.now();
