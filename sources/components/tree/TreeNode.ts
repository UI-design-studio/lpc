...
import { t, tTreeLabel, tItemName } from "../../i18n/index.ts";
...
function renderItem(itemId: string, meta: ItemMerged, ctx: ItemListCtx) {
  ...
  const displayName = tItemName(meta.name);
  ...
  if (!catalog.isCreditsReady()) {
    licensesText = t("tooltip.license_loading");
  } else {
  ...
  if (!isCompatible) {
    const issues: string[] = [];
    if (!isLicenseCompatibleFlag) issues.push(t("selections.licenses").replace("：", ""));
    if (!isAnimCompatibleFlag) issues.push(t("selections.animations").replace("：", ""));
...
export const TreeNode: m.Component<TreeNodeAttrs> = {
  view(vnode) {
    ...
    if (!isNodeAnimCompatible) {
      tooltipText = `${t("tooltip.incompatible_animations")}\n`;
    }
    ...
    const isExpanded =
      ...
      false;
    const displayName = tTreeLabel(name, node.label, capitalize);
