...
export const exportSplitAnimations = async (
  ...
  try {
    ...
    if (failedStandard.length > 0 || failedCustom.length > 0) {
      alert(
        tInterp("alert.export_complete_with_issues", { failed: failedStandard.join(", ") }),
      );
    } else {
      alert(t("alert.export_complete"));
    }
  } catch (err) {
    console.error("Export failed:", err);
    alert(tInterp("alert.export_failed", { message: (err as Error).message }));
  } finally {
    endZipExportUiSuspend();
    if (state) {
...
export const exportSplitItemSheets = async (
  ...
  try {
    ...
    if (failedItems.length > 0) {
      alert(
        tInterp("alert.export_items_issues", { failed: failedItems.join(", ") }),
      );
    } else {
      alert(t("alert.export_complete"));
    }
...
export const exportSplitItemAnimations = async (
  ...
  try {
    ...
    if (failedCount > 0) {
      let msg = t("alert.export_complete") + "\n";
  ...
  try {
    ...
    if (totalFailed > 0) {
      ...
      if (failedAnimations.length > 0) {
        msg += tInterp("alert.failed_standard_animations", { failed: failedAnimations.join(", ") }) + "\n";
      }
      if (failedCustom.length > 0) {
        msg += tInterp("alert.failed_custom_animations", { failed: failedCustom.join(", ") }) + "\n";
      }
      alert(msg);
    } else {
      alert(t("alert.export_individual_complete"));
    }
  } catch (err) {
