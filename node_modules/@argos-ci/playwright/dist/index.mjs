...
import { getGitRepositoryPath, getMetadataPath, getScreenshotName, normalizeBaseNames, readVersionFromPackage, validateThreshold, writeMetadata } from "@argos-ci/util";
...
function checkIsUsingArgosReporter(testInfo) {
	...
	return testInfo.config.reporter.some((reporter) => reporter[0].includes("@argos-ci/playwright") || reporterPath !== null && reporter[0] === reporterPath);
}
...
*/
function getSnapshotNames(name, testInfo, baseNames) {
	const prefix = testInfo?.project.name ? `${testInfo.project.name}/` : "";
	const prefixedName = `${prefix}${name}`;
	const prefixedBaseNames = baseNames && baseNames.length > 0 ? baseNames.map((baseName) => `${prefix}${baseName}`) : null;
	if (testInfo && testInfo.repeatEachIndex > 0) return {
		name: `${prefixedName} repeat-${testInfo.repeatEachIndex}`,
		baseNames: prefixedBaseNames ?? [prefixedName]
	};
	return {
		name: prefixedName,
		baseNames: prefixedBaseNames
	};
}
/**
...
async function getPathAndMetadata(args) {
	...
	metadata.transient = {};
	if (names.baseNames) metadata.transient.baseName = names.baseNames.map((baseName) => `${baseName}${extension}`);
	return {
		metadata,
		path
	};
...
async function argosScreenshot(handler, name, options = {}) {
	const { element, has, hasText, hasNot, hasNotText, viewports, argosCSS: _argosCSS, root = DEFAULT_SCREENSHOT_ROOT, ariaSnapshot, disableHover = true, baseName, ...playwrightOptions } = options;
	...
	const stabilizeAndScreenshot = async (name, baseNames) => {
			...
			(async () => {
				...
				const snapshotMetadata = {
					...
					transient: {
						...
						...names.baseNames ? { baseName: names.baseNames.map((baseName) => screenshotToSnapshotPath(`${baseName}${PNG_EXTENSION}`)) } : {}
					}
				};
	...
	if (viewports) {
		...
		for (const viewport of viewports) {
			...
			const attachments = await stabilizeAndScreenshot(getScreenshotName(name, { viewportWidth: viewportSize.width }), baseNames?.map((baseName) => getScreenshotName(baseName, { viewportWidth: viewportSize.width })) ?? null);
