...
*/
const RECOMMENDED_CHROMIUM_ARGS = ["--disable-lcd-text", "--font-render-hinting=none"];
/**
* Warn if the Playwright config is missing the recommended Chromium launch
* options used to stabilize screenshots.
*/
function checkLaunchOptions(config) {
	for (const project of config.projects) {
		if ((project.use.browserName ?? "chromium") !== "chromium") continue;
		const args = project.use.launchOptions?.args ?? [];
		const missing = RECOMMENDED_CHROMIUM_ARGS.filter((arg) => !args.includes(arg));
		if (missing.length > 0) {
			const projectLabel = project.name || "default";
			console.warn(chalk.yellow(`⚠️  Argos: Playwright project "${projectLabel}" is missing recommended launchOptions args: ${missing.join(", ")}.\n   Add them to stabilize font rendering and get consistent screenshots. See https://argos-ci.com/docs/reference/playwright`));
		}
	}
}
