...
var TinyGesture = class TinyGesture {
		...
		if (event.type !== "mousedown") {
			...
			if ((didTouch1 && event.changedTouches.length > 1 || !didTouch1) && !this.touch2) {
				this.touch2 = [...event.changedTouches].find((touch) => {
					var _a;
					return touch.identifier !== ((_a = this.touch1) === null || _a === void 0 ? void 0 : _a.identifier);
				}) || null;
	...
	onTouchMove(event) {
		...
		if (event.type !== "mousemove") {
			...
			touch2 = [...event.changedTouches].find((touch) => {
				var _a;
				return touch.identifier === ((_a = this.touch2) === null || _a === void 0 ? void 0 : _a.identifier);
			});
		...
		if (event.type === "mousemove" || touch1) {
			...
			if (Math.max(absTouchMoveX, absTouchMoveY) > this.opts.pressThreshold) clearTimeout((_g = this.longPressTimer) !== null && _g !== void 0 ? _g : void 0);
		...
		if (event.type === "mouseup" || touch1) {
			...
			if (absX > this.thresholdX || absY > this.thresholdY || this.opts.diagonalSwipes && (absDistance > this.thresholdX || absDistance > this.thresholdY)) {
				...
				if (!this.opts.diagonalSwipes || diagonal < Math.tan((45 - this.opts.diagonalLimit) * Math.PI / 180) || diagonal > Math.tan((45 + this.opts.diagonalLimit) * Math.PI / 180)) {
					if (absX >= absY) this.swipedVertical = false;
					if (absY > absX) this.swipedHorizontal = false;
				}
				if (this.swipedHorizontal) {
					if (x < 0) {
						if (((_f = this.velocityX) !== null && _f !== void 0 ? _f : 0) < -this.opts.velocityThreshold || distance > this.disregardVelocityThresholdX) this.fire("swipeleft", event);
					} else if (((_g = this.velocityX) !== null && _g !== void 0 ? _g : 0) > this.opts.velocityThreshold || distance > this.disregardVelocityThresholdX) this.fire("swiperight", event);
				}
				if (this.swipedVertical) {
					if (y < 0) {
						if (((_h = this.velocityY) !== null && _h !== void 0 ? _h : 0) < -this.opts.velocityThreshold || distance > this.disregardVelocityThresholdY) this.fire("swipeup", event);
					} else if (((_j = this.velocityY) !== null && _j !== void 0 ? _j : 0) > this.opts.velocityThreshold || distance > this.disregardVelocityThresholdY) this.fire("swipedown", event);
				}
...
TinyGesture.defaults = {
	threshold: (type, _self) => Math.max(25, Math.floor(.15 * (type === "x" ? window.innerWidth || document.body.clientWidth : window.innerHeight || document.body.clientHeight))),
	velocityThreshold: 10,
	disregardVelocityThreshold: (type, self) => Math.floor(.5 * (type === "x" ? self.element.clientWidth : self.element.clientHeight)),
	pressThreshold: 8,
